...
`srt` is a single-form Delphi 7 application: `BtnProcurar` discovers a router through ARP output, `BtnAtivar` requests a UPnP port mapping, and `RunDosInMemo` runs console commands invisibly through an anonymous pipe into a `TMemo`. It is a preserved archive, discontinued in 2014.
...
## What matters most in `srt`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **Opening a port on someone's router is the whole point and the whole risk.** Any change to the UPnP mapping — the external port range (`SePorta`, 1–1024, default 900), the protocol, or the internal target — changes what becomes reachable from the internet. Say so explicitly and record it in the changelog. **Critical** if the change widens exposure silently.
- **The hard-coded MAC address (`88-f7-c7-6e-bc-94`) is a device fingerprint.** It is documented as needing an edit per router. Leaving a *different* real MAC in a commit identifies a real device — flag it. Making the value configurable would be a genuine improvement, and it is a behavioural change.
...
```bash
# open SRT.dpr in Borland Delphi 7 and compile (Ctrl+F9)
# run SRT.exe on a Windows machine attached to the target network
```
