...
- fixed the `.chlog.yaml` header comment, which told the reader to keep the kinds in step with an AI-assistant block in `CLAUDE.md` — a file this repository does not have. The block lives in `.github/copilot-instructions.md`, and the comment now points there.
